...
public:
    ...
    {
        string ceva = "Pentru a reintroduce numarul de pagini apasati 'r'";
        string ceva1 = "Pentru a anula introducere cartii apasati 'n'";
        char refacere;
        if(this->numar_pagini != nr_pagini)
        {
            cout<<"\nNumarul de pagini introdus este invalid, cartea trebuie sa aibe 300 de pagini."<<endl;
            cout<<ceva<<endl<<ceva1<<endl;
        }
        ...
        while(this->numar_pagini != nr_pagini)
        {
            ...
            {
                cout<<"\n!!! COMANDA INTRODUSA ESTE INVALIDA !!!"<<endl<<ceva<<endl<<ceva1<<endl;
                cin.clear();
                cin.ignore(100, '\n');
            }
        }
...
public:
    ...
    void Afisare_lista_fisier()
    {
        ofstream file;
        file.open("FisierCarti.txt");
        int j = 1;
        for(auto const& i: lista_carti)
        {
            file<<"Cartea "<<j<<" este: ";
            file<<i.Get_Titlu()<<" de "<<i.Get_Nume();
            j++;
            file<<endl;
        }

        file<<"Lista contine:"<<lista_carti.size()<<" carti"<<endl;
...
void Afisare_carti_duble(vector<Carte>& vector_carti)
{
    for(int i = 0; i < vector_carti.size(); i++)
    {
        cout<<"Cartea "<<i+1<<" introdusa pentru a doua oara este: "<<vector_carti[i];
    }
}

void Afisare_coduri(set<int> coduri)
{
    set<int>::iterator i;
    cout<<"Codurile sunt: "<<endl;
    for(i = coduri.begin(); i != coduri.end(); ++i )
    {
        cout<<*i<<endl;
    }
}

void Afisare_map(map<string, string> lista_de_carti2)
{
    map<string, string>::iterator i;
    cout<<"Cartile sunt: "<<endl;
    for(i = lista_de_carti2.begin(); i != lista_de_carti2.end(); ++i )
    {
        cout<<(*i).first<<endl;
        cout<<(*i).second<<endl;
    }
...
public:
    void Afisare()
    {
        //system("Color 2E"); ///schimba culoarea consolei (numarul = background, litera = textul) merge doar in windows
        ...
        cout<<"3 - Deschidere lista de carti personala"<<endl;
        ...
        cout<<"9 - Afisare lista de carti in fisier"<<endl;
        cout<<"10 - Citire din fisier"<<endl;
        cout<<"11 - Oprire program"<<endl;
        cout<<"\n";
    }
    ...
    void Verificare_Comanda()
    {
        ...
        vector<Carte> vector_carti_duble;
        set<int> coduri;
        map<string, string> lista_carti2;
        string stergere;
        ...
        while(optiune != 11)
        {
            ...
            {
                cin>>optiune;
                if(cin.fail() || optiune >= 12 || optiune < 0)
                {
                    throw optiune;
                }
                ...
                if(optiune == 1)
                {
                    ...
                    if(tip == "nu")
                    {
                        ...
                        if(lista_carti.Verificare_carti(carte_adaugata) &&  verif == true)
                        {
                            ...
                            coduri.insert(carte_adaugata.Get_cod_bare());
                            string nume = lista_carti.GetLista().back().Get_Titlu();
                            string titlu = lista_carti.GetLista().back().Get_Nume();
                            lista_carti2.insert(pair<string, string>(titlu, nume));
                        ...
                        {
                            vector_carti_duble.push_back(carte_adaugata);
                ...
                if(optiune == 9)
                {
                    lista_carti.Afisare_lista_fisier();
                ...
                if(optiune == 10)
                {
                    ifstream file;
                    file.open("FisierCarti.txt");
                    string linie;       /// linie e folosit ca sa imi salveze temporar si afiseze linia din fisier

                    while(!file.eof()) /// file.eof returneaza un bool, true daca ajunge la finalul fisierului si fals altfel
                    {
                        getline(file, linie);
                        cout<<linie<<endl;
                    }
        ...
        //Afisare_map(lista_carti2); ///afiseaza map-ul de carti
        //Afisare_coduri(coduri); ///afiseaza codurile de bare ale cartilor
        //Afisare_carti_duble(vector_carti_duble); ///functie ce afiseaza cartile introduse de mai multe ori(lista principala nu accepta dubluri si am facut un vector in care se pastreaza totusi aceste carti introduse de mai multe ori)
    }
};
